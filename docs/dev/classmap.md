...
| `App\Setup\SetupHelpLoader` | `src/Setup/SetupHelpLoader.php` | Loads contextual help entries from project-root `help.json` (with locale overrides), merges fallbacks, and preserves `target` metadata for field-level tooltips | Used by installer controller/templates |
...
| `App\Routing\RouterDefaultUriSubscriber` | `src/Routing/RouterDefaultUriSubscriber.php` | Sets router default URI for CLI contexts, preferring the stored system setting and falling back to `DEFAULT_URI` | Subscribed to console command events |
